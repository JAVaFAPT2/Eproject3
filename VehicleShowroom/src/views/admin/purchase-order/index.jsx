...
export default function PurchaseOrderPage() {
  ...
  const filtered = useMemo(
    () =>
      data.filter((v) =>
        v.orderNumber.toLowerCase().includes(search.toLowerCase()) ||
        v.modelNumber.toLowerCase().includes(search.toLowerCase())
      ),
    [data, search],
  );
  ...
  const handleDelete = async () => {
    try {
      await PurchaseOrderService.delete(confirm.target.id);
