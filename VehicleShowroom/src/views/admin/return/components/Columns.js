...
export const getColumns = ({ onEdit, onDelete, textColor }) => [
  c.accessor('returnId', { header: 'RETURN ID', cell: (i) => <Text color={textColor}>{i.getValue()}</Text> }),
  c.accessor('orderId', { header: 'ORDER ID', cell: (i) => <Text>{i.getValue()}</Text> }),
  c.accessor('reason', { header: 'REASON', cell: (i) => <Text>{i.getValue()}</Text> }),
