...
export default function ReturnPage() {
  ...
  const filtered = useMemo(
    () =>
      data.filter((v) =>
        v.returnId.toLowerCase().includes(search.toLowerCase()) ||
        v.orderId.toLowerCase().includes(search.toLowerCase())
      ),
    [data, search],
  );
  ...
  const handleDelete = async () => {
    try {
      await ReturnService.delete(confirm.target.id);
  ...
  return (
    <Box pt={{ base: '130px', md: '80px' }}>
      ...
      <ConfirmDialog
        ...
        title="Delete Return"
        message={`Delete ${confirm.target?.returnId}?`}
      />
      <Card bg={bgColor}>
